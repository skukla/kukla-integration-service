  ...
  if (!processedParams.NODE_ENV && !process.env.NODE_ENV) {
    if (!rawOutput) {
      ...
      try {
        ...
        envSpinner.succeed(core.formatSpinnerSuccess(`Environment detected: ${capitalizedEnv}`));
      } catch (error) {
...
async function executeEnhancedTest(actionName, processedParams, displayActionResults) {
  ...
  testSpinner.succeed(core.formatSpinnerSuccess(`Action tested: ${actionName}`));
