...
async function getProductsMeshBusinessLogic(context) {
  ...
  // Use products domain workflow for unified mesh export
  const includeCSV = (webActionParams.format || extractedParams.format) !== 'json';
  ...
  // Use files domain workflow for CSV export (CSV already generated by unified workflow)
