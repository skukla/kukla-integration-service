...
async function createCache(params, config, logger) {
  ...
  if (enabled) {
    try {
      ...
      // Check kill switch BEFORE using cache
      try {
        const killSwitch = await state.get('cache_kill_switch');
        if (killSwitch?.value === 'true') {
          logger.warn('⚠️ Cache kill switch activated - all caching disabled');
          return {
            state: null,
            enabled: false,
            config,
        ...
        // If we can't check kill switch, proceed with normal caching
        logger.debug('Could not check kill switch, proceeding normally');
      }
  ...
  return {
    ...
    delete: (endpoint, params, token) => invalidateCache(state, endpoint, params, token, enabled),
  };
}
