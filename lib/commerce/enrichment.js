...
async function enrichProducts(products, config, bearerToken, cache = null, logger = null) {
  ...
  // Step 3: Fetch all enrichment data in parallel with caching
  let categoryResults = [];
  ...
  if (categoryPromises.length > 0 && cache) {
    ...
    if (cachedCategories) {
      ...
      cacheHits++;
      if (logger) logger.debug('Categories cache HIT');
    } else {
      categoryResults = await Promise.all(categoryPromises);
      // Cache the results
      if (categoryResults.length > 0) {
        await cache.put('categories', cacheKey, bearerToken, categoryResults[0]);
      }
      if (logger) logger.debug('Categories cache MISS');
    }
  } else {
    categoryResults = await Promise.all(categoryPromises);
  }
  ...
  if (inventoryPromises.length > 0 && cache) {
    ...
    if (cachedInventory) {
      ...
      cacheHits++;
      if (logger) logger.debug('Inventory cache HIT');
    } else {
      inventoryResults = await Promise.all(inventoryPromises);
      // Cache the results
      if (inventoryResults.length > 0) {
        await cache.put('inventory', cacheKey, bearerToken, inventoryResults[0]);
      }
      if (logger) logger.debug('Inventory cache MISS');
    }
  } else {
    inventoryResults = await Promise.all(inventoryPromises);
  }
