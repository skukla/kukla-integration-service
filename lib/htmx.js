...
function generateFileBrowserHTML(files, params = {}) {
  ...
  const fileRows = files
    .map((file) => {
      // Build download URL dynamically using Adobe I/O Runtime context (no user configuration needed)
      const host = params.__OW_API_HOST;
      const namespace = params.__OW_NAMESPACE;
      // Extract package name from action name (format: "package/action")
      const packageName = params.__OW_ACTION_NAME.split('/')[0];
      const downloadUrl = `https://${host}/api/v1/web/${namespace}/${packageName}/download-file?fileName=${encodeURIComponent(file.name)}`;
